...
export default function DashboardPremium() {
  ...
  // =========================================================================
  // BACA DATA DARI NODE 'notes' DI FIREBASE
  // =========================================================================
  useEffect(() => {
    setLoading(true);
  ...
  // =========================================================================
  // SISTEM HAPUS (HANYA SUPER ADMIN)
  // =========================================================================
  ...
  // =========================================================================
  // ENGINE CETAK PDF F4 RESMI (ANTI TERPOTONG / HANCUR)
  // =========================================================================
  const handleCetakPDF = async (item: Notulen) => {
    ...
    const formatHtmlPDF = (textData?: string | string[] | null) => {
      ...
      return text.split('\n').filter(p => p.trim() !== '').map(p => {
        ...
        let paddingLeft = isMainPoint ? '30px' : isSubPoint ? '45px' : '0px';
        let textIndent = isMainPoint ? '-30px' : isSubPoint ? '-20px' : '0px';
        let fontWeight = isMainPoint ? 'bold' : 'normal';
        let textTransform = isMainPoint ? 'uppercase' : 'none';
        let marginTop = isMainPoint ? '14px' : '6px'; 

        // ANTI TERPOTONG: Menambahkan page-break-inside: avoid pada setiap baris/paragraf
        return `<div style="page-break-inside: avoid; margin-top: ${marginTop}; margin-bottom: 6px; text-align: justify; text-justify: inter-word; line-height: 1.5; padding-left: ${paddingLeft}; text-indent: ${textIndent}; font-weight: ${fontWeight}; text-transform: ${textTransform}; font-family: 'Times New Roman', serif; font-size: 12pt;">${cleanText}</div>`;
      }).join('');
    };
    
    printContainer.innerHTML = `
      <div id="print-capture-area" style="padding: 25mm 20mm 20mm 25mm; font-family: 'Times New Roman', Times, serif; color: #000; background: #fff; width: 215mm; box-sizing: border-box;">
        
        <div style="text-align: center; border-bottom: 3px double #000; padding-bottom: 12px; margin-bottom: 25px; page-break-inside: avoid;">
        ...
        <div style="margin-bottom: 25px; page-break-inside: avoid;">
          <h3 style="font-size: 12pt; font-weight: bold; text-transform: uppercase; margin-bottom: 12px;">I. PEMBUKAAN & IDENTITAS RAPAT</h3>
          <table style="width: 100%; border-collapse: collapse; font-size: 12pt; table-layout: fixed;">
            <tr>
              <td style="width: 28%; padding: 4px 0; font-weight: bold; vertical-align: top;">Judul Kegiatan</td>
            ...
            <tr>
              <td style="padding: 4px 0; font-weight: bold; vertical-align: top;">Hari / Tanggal</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: center;">:</td>
              <td style="padding: 4px 0; vertical-align: top;">${item.tanggal || '-'}</td>
            </tr>
            <tr>
              <td style="padding: 4px 0; font-weight: bold; vertical-align: top;">Waktu Pelaksanaan</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: center;">:</td>
              <td style="padding: 4px 0; vertical-align: top;">${item.waktu_mulai || '-'} s/d ${item.waktu_selesai || 'Selesai'} WITA</td>
            </tr>
            <tr>
              <td style="padding: 4px 0; font-weight: bold; vertical-align: top;">Tempat / Lokasi</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: center;">:</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: justify;">${item.tempat || '-'}</td>
            </tr>
            <tr>
              <td style="padding: 4px 0; font-weight: bold; vertical-align: top;">Pimpinan Rapat</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: center;">:</td>
              <td style="padding: 4px 0; vertical-align: top;">${item.pimpinan_rapat || '-'}</td>
            </tr>
            <tr>
              <td style="padding: 4px 0; font-weight: bold; vertical-align: top;">Notulis / Pencatat</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: center;">:</td>
              <td style="padding: 4px 0; vertical-align: top;">${item.notulis || '-'}</td>
            </tr>
            <tr>
              <td style="padding: 4px 0; font-weight: bold; vertical-align: top;">Daftar Hadir Peserta</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: center;">:</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: justify; word-wrap: break-word;">${item.peserta || '-'}</td>
            </tr>
            <tr>
              <td style="padding: 4px 0; font-weight: bold; vertical-align: top;">Agenda Pembahasan</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: center;">:</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: justify; word-wrap: break-word;">${item.agenda || '-'}</td>
            </tr>
          </table>
        </div>

        <div style="margin-bottom: 25px;">
          <h3 style="font-size: 12pt; font-weight: bold; text-transform: uppercase; margin-bottom: 12px; page-break-inside: avoid;">II. HASIL PEMBAHASAN / NOTULENSI</h3>
          <div>${formatHtmlPDF(item.isi_notulen)}</div>
        </div>

        <div style="margin-bottom: 25px; page-break-inside: avoid;">
          <h3 style="font-size: 12pt; font-weight: bold; text-transform: uppercase; margin-bottom: 12px;">III. KESIMPULAN EKSEKUTIF</h3>
          <div>${formatHtmlPDF(item.kesimpulan || item.ai_structured?.ringkasan)}</div>
        </div>

        <div style="margin-bottom: 35px; page-break-inside: avoid;">
          <h3 style="font-size: 12pt; font-weight: bold; text-transform: uppercase; margin-bottom: 12px;">IV. RENCANA TINDAK LANJUT (RTL)</h3>
          <div>${formatHtmlPDF(item.tindak_lanjut || item.ai_structured?.tindak_lanjut)}</div>
        </div>

        <table style="width: 100%; border-collapse: collapse; page-break-inside: avoid; table-layout: fixed; margin-top: 50px;">
          <tr>
            <td style="width: 55%;"></td>
            <td style="width: 45%; text-align: center; line-height: 1.5; font-size: 11pt;">
              ...
              <p style="margin: 4px 0 0 0;">Pimpinan Rapat</p>
            </td>
          </tr>
        </table>
  ...
  return (
    <>
      ...
      <div className="min-h-screen w-full bg-slate-50 font-sans text-slate-800 pb-12 selection:bg-yellow-200">
        ...
        {/* ========================================================= */}
        {/* VIEW DETAIL MODAL PREVIEW (BARU) */}
        {/* ========================================================= */}
        {viewItem && (
            ...
            <div className="bg-white rounded-3xl w-full max-w-3xl relative overflow-hidden shadow-2xl my-auto flex flex-col max-h-[90vh]">
              ...
              <div className="p-6 overflow-y-auto custom-scrollbar">
                ...
                <div className="space-y-8">
                  <div>
                    <h3 className="text-xs font-extrabold uppercase tracking-widest text-slate-800 mb-3 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-blue-500"></span> Hasil Pembahasan</h3>
                    <div className="text-sm text-slate-600 leading-loose whitespace-pre-wrap text-justify bg-white">{viewItem.isi_notulen || '-'}</div>
                  </div>
                  <div className="p-5 rounded-2xl bg-yellow-50/50 border border-yellow-100">
                    <h3 className="text-xs font-extrabold uppercase tracking-widest text-yellow-800 mb-3 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-yellow-500"></span> Kesimpulan Eksekutif</h3>
                    <div className="text-sm text-yellow-900/80 leading-relaxed whitespace-pre-wrap">{viewItem.kesimpulan || viewItem.ai_structured?.ringkasan || '-'}</div>
                  </div>
                  <div>
                    <h3 className="text-xs font-extrabold uppercase tracking-widest text-slate-800 mb-3 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-emerald-500"></span> Tindak Lanjut (RTL)</h3>
                    <div className="text-sm text-slate-600 leading-relaxed whitespace-pre-wrap">{viewItem.tindak_lanjut || viewItem.ai_structured?.tindak_lanjut || '-'}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3 sticky bottom-0">
                ...
                <button 
                  ...
                  className="px-6 py-2.5 rounded-xl bg-yellow-400 text-yellow-950 text-xs font-bold hover:bg-yellow-500 transition-colors shadow-lg flex items-center gap-2"
                >
        ...
        {/* ========================================================= */}
        {/* MODAL OTORISASI ADMIN & SUPER ADMIN */}
        {/* ========================================================= */}
        {showPinModal && (
        ...
        {/* ========================================================= */}
        {/* MODAL HAPUS PERMANEN (KHUSUS SUPER ADMIN) */}
        {/* ========================================================= */}
        {deleteId && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            <div className="bg-white rounded-2xl p-6 w-full max-w-sm border border-red-100 shadow-2xl">
              ...
              <p className="text-slate-500 text-sm leading-relaxed mb-6">Tindakan ini hanya dapat dilakukan oleh Super Admin dan data arsip akan dihapus secara permanen dari server.</p>
            ...
            <div className="flex gap-3 items-center">
              ...
              <button 
                ...
                className={`p-2.5 rounded-xl transition-all border ${
                  isSuperAdmin ? 'bg-red-50 border-red-300 text-red-600 shadow-md shadow-red-400/10' :
                  isAdmin ? 'bg-yellow-50 border-yellow-400 text-yellow-600 shadow-md shadow-yellow-400/20' : 
                  'bg-slate-50 border-slate-200 text-slate-400 hover:text-yellow-500 hover:bg-slate-100'
                }`}
                title={userRole !== 'public' ? `Mode ${userRole.toUpperCase()} Aktif` : "Login Akses"}
              >
                {isAdmin ? (
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
              {paginatedData.map((item) => {
                ...
                // SISTEM STATUS KUNCI: DEFAULTNYA ADALAH TERKUNCI JIKA DATA LAMA/KOSONG
                const isLockedDoc = item.isLocked !== false; 
                // IZINKAN EDIT JIKA TERBUKA ATAU JIKA USER ADALAH ADMIN/SUPERADMIN
                ...
                return (
                  <div key={item.id} className={`bg-white rounded-2xl p-6 border ${item.status === 'rahasia' ? 'border-yellow-400 shadow-[0_0_15px_rgba(250,204,21,0.2)]' : 'border-slate-200 shadow-sm'} hover:shadow-xl hover:-translate-y-1 hover:border-yellow-400 transition-all duration-300 flex flex-col justify-between group relative overflow-hidden`}>
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-4 mt-1">
                        ...
                        <div className="flex items-center gap-1.5">
                          {/* TOMBOL TOGGLE LOCK KHUSUS ADMIN/SUPER ADMIN */}
                          <button
                            onClick={() => handleToggleLock(item)}
                            disabled={!isAdmin}
                            className={`text-[9px] font-black tracking-wider px-2 py-1 rounded-md uppercase border transition-all ${
                            ...
                            title={isAdmin ? "Ubah Status Kunci Dokumen" : "Status Kunci"}
                          >
                            {isLockedDoc ? '🔒 KUNCI' : '🔓 BUKA'}
                          </button>
                    ...
                    <div className="pt-4 mt-2 border-t border-slate-100">
                      ...
                      {/* ACTION GRID: LIHAT, EDIT, CETAK PDF */}
                      <div className="grid grid-cols-3 gap-2.5">
                        <button 
                          onClick={() => setViewItem(item)} 
                          className="py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-600 text-[11px] font-bold text-center hover:bg-slate-100 transition-all flex items-center justify-center gap-1.5"
                        >
                          Lihat
                        </button>
                        ...
                        {canUserEdit ? (
                          ...
                          <button 
                            disabled 
                            className="py-2.5 bg-slate-100 border border-slate-200 rounded-xl text-slate-400 text-[11px] font-bold text-center cursor-not-allowed opacity-60 flex items-center justify-center gap-1.5"
                            title="Dokumen Terkunci. Hanya Admin yang bisa mengedit."
                          >
                            🔒 Edit
                          </button>
                        )}
                        ...
                        <button 
                          ...
                          className="py-2.5 bg-yellow-400 text-yellow-950 rounded-xl text-[11px] font-extrabold hover:bg-yellow-500 transition-all shadow-md shadow-yellow-400/20 disabled:opacity-50 flex items-center justify-center gap-1.5"
                        >
                          {printingId === item.id ? 'Memproses' : 'Print'}
                        </button>
                      </div>

                      {/* HAK AKSES KHUSUS SUPER ADMIN: HAPUS PERMANEN */}
                      {isSuperAdmin && (
                        <div className="flex mt-2.5 pt-2.5 border-t border-dashed border-slate-200 animate-fade-in">
                          <button onClick={() => setDeleteId(item.id)} className="w-full py-2 bg-red-50 text-red-600 rounded-lg text-[10px] font-bold uppercase hover:bg-red-100 transition-colors tracking-wide flex justify-center items-center gap-2">
      ...
      {/* KODE STYLE ASLI UNTUK CUSTOM SCROLLBAR (TIDAK DIHILANGKAN) */}
