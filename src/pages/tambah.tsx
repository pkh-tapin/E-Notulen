...
export default function DashboardPremium() {
  ...
  const handleCetakPDF = async (item: Notulen) => {
    ...
    const formatHtmlPDF = (textData?: string | string[] | null) => {
      ...
      if (!text || text.trim() === '') return '<div style="margin-bottom: 8px; font-family: \'Times New Roman\', serif; font-size: 12pt;">-</div>';
      
      return text.split('\n').filter(p => p.trim() !== '').map(p => {
        let cleanText = p.replace(/\*/g, '').trim();
        
        // Pola pendeteksian sub-poin resmi (Contoh: 1., a., b., atau tanda strip)
        let isMainPoint = /^\d+\.\s/.test(cleanText);
        let isAlphaSub = /^[a-zA-Z]\.\s/.test(cleanText);
        let isBulletPoint = cleanText.startsWith('-');
        ...
        if (isMainPoint) {
          paddingLeft = '30px';
          textIndent = '-30px';
        } else if (isAlphaSub || isBulletPoint) {
          paddingLeft = '45px';
          textIndent = '-20px';
        }
        ...
        return `<div style="page-break-inside: avoid; margin-top: ${marginTop}; margin-bottom: 6px; text-align: justify; text-justify: inter-word; line-height: 1.5; padding-left: ${paddingLeft}; text-indent: ${textIndent}; font-family: 'Times New Roman', serif; font-size: 12pt; font-weight: ${fontWeight}; text-transform: ${textTransform}; color: #000; word-break: break-word;">${cleanText}</div>`;
      }).join('');
    };
    
    printContainer.innerHTML = `
      <div id="print-capture-area" style="padding: 25mm 20mm 20mm 30mm; font-family: 'Times New Roman', Times, serif; color: #000; background: #fff; width: 215mm; box-sizing: border-box;">
        
        <div style="text-align: center; border-bottom: 3px double #000; padding-bottom: 10px; margin-bottom: 25px; page-break-inside: avoid;">
          <h1 style="margin: 0; font-size: 14pt; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5px; font-family: 'Times New Roman', serif;">LAPORAN HASIL KEGIATAN & NOTULENSI RAPAT</h1>
          <h2 style="margin: 4px 0 0 0; font-size: 12pt; font-weight: bold; text-transform: uppercase; font-family: 'Times New Roman', serif;">SUMBER DAYA MANUSIA PROGRAM KELUARGA HARAPAN (PKH)</h2>
          <p style="margin: 4px 0 0 0; font-size: 12pt; font-weight: bold; text-transform: uppercase; font-family: 'Times New Roman', serif;">KABUPATEN TAPIN</p>
        </div>

        <div style="margin-bottom: 25px; page-break-inside: avoid;">
          <h3 style="font-size: 12pt; font-weight: bold; text-transform: uppercase; margin-bottom: 12px; font-family: 'Times New Roman', serif;">I. DOKUMEN IDENTITAS KEGIATAN</h3>
          <table style="width: 100%; border-collapse: collapse; font-size: 12pt; font-family: 'Times New Roman', serif; table-layout: fixed;">
            <tr>
            ...
            <tr>
              <td style="padding: 4px 0; vertical-align: top;">Hari / Tanggal</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: center;">:</td>
              <td style="padding: 4px 0; vertical-align: top;">${item.tanggal || '-'}</td>
            </tr>
            <tr>
              <td style="padding: 4px 0; vertical-align: top;">Waktu Pelaksanaan</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: center;">:</td>
              <td style="padding: 4px 0; vertical-align: top;">${item.waktu_mulai || '-'} s/d ${item.waktu_selesai || 'Selesai'} WITA</td>
            </tr>
            <tr>
              <td style="padding: 4px 0; vertical-align: top;">Tempat / Lokasi</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: center;">:</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: justify;">${item.tempat || '-'}</td>
            </tr>
            <tr>
              <td style="padding: 4px 0; vertical-align: top;">Pimpinan Rapat</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: center;">:</td>
              <td style="padding: 4px 0; vertical-align: top;">${item.pimpinan_rapat || '-'}</td>
            </tr>
            <tr>
              <td style="padding: 4px 0; vertical-align: top;">Notulis / Pencatat</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: center;">:</td>
              <td style="padding: 4px 0; vertical-align: top;">${item.notulis || '-'}</td>
            </tr>
            <tr>
              <td style="padding: 4px 0; vertical-align: top;">Daftar Hadir Peserta</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: center;">:</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: justify; word-break: break-word;">${item.peserta || '-'}</td>
            </tr>
            <tr>
              <td style="padding: 4px 0; vertical-align: top;">Agenda Pembahasan</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: center;">:</td>
              <td style="padding: 4px 0; vertical-align: top; text-align: justify; word-break: break-word;">${item.agenda || '-'}</td>
            </tr>
          </table>
        </div>

        <div style="margin-bottom: 25px;">
          <h3 style="font-size: 12pt; font-weight: bold; text-transform: uppercase; margin-bottom: 12px; font-family: 'Times New Roman', serif; page-break-inside: avoid;">II. JALANNYA RAPAT DAN PEMBAHASAN</h3>
          <div>${formatHtmlPDF(item.isi_notulen)}</div>
        </div>

        <div style="margin-bottom: 25px; page-break-inside: avoid;">
          <h3 style="font-size: 12pt; font-weight: bold; text-transform: uppercase; margin-bottom: 12px; font-family: 'Times New Roman', serif;">III. KESIMPULAN AKHIR RAPAT</h3>
          <div>${formatHtmlPDF(item.kesimpluan || item.kesimpulan || item.ai_structured?.ringkasan)}</div>
        </div>

        <div style="margin-bottom: 35px; page-break-inside: avoid;">
          <h3 style="font-size: 12pt; font-weight: bold; text-transform: uppercase; margin-bottom: 12px; font-family: 'Times New Roman', serif;">IV. RENCANA TINDAK LANJUT (RTL)</h3>
          <div>${formatHtmlPDF(item.tindak_lanjut || item.ai_structured?.tindak_lanjut)}</div>
        </div>

        <table style="width: 100%; border-collapse: collapse; page-break-inside: avoid; font-family: 'Times New Roman', serif; font-size: 12pt; table-layout: fixed; margin-top: 40px;">
          <tr>
            <td style="width: 55%;"></td>
            <td style="width: 45%; text-align: center; line-height: 1.5;">
    ...
    // SCALE DAN UKURAN DIKONCI KHUSUS F4 LEGAL INDONESIA (215mm x 330mm)
    const opt = {
      margin: 0, 
